...
bool rotationCompleted = false; // 新增标志位
...
void handle_return_to_0() {
  targetAngle = 0;
  servoState_1 = 2; // 设置为返回到初始位置
  handle_stop_capturing(); // 自动关闭拍摄
...
void setup() 
{
  ...
  digitalWrite(LED_GPIO, LOW); // 初始状态为关闭
  ...
  server.on("/", HTTP_GET, [](){
    server.send_P(200, "text/html", R"rawliteral(
        ...
        <script>
          ...
          function setAngle() {
            ...
            fetch('/set_angle?angle=' + angle).then(response => response.text()).then(text => {
              alert(text);
              rotationCompleted = false; // 重置标志位
              setTimeout(() => {
                rotationCompleted = true; // 延时设置为 true
                startCapturing(); // 延时启动拍摄
              }, 5000); // 等待 5 秒
            });
          }
          ...
          function toggleLED() {
            fetch('/toggle_led').then(response => response.text()).then(text => {
              alert(text);
            });
          }
      ...
      <body>
        ...
        <button onclick="toggleLED()">开启/关闭闪光灯</button>
        <div id="photos"></div>
        <br><br>
      </body>
      </html>
  ...
  server.on("/return_to_0", HTTP_GET, handle_return_to_0);
  ...
  server.on("/toggle_led", HTTP_GET, [](){
    ...
    server.send(200, "text/plain", ledState ? "LED 已开启" : "LED 已关闭");
  });
